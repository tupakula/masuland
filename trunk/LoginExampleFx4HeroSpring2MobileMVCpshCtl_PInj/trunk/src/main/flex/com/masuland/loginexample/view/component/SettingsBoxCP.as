...
//				ddl_layout.selectedItem = _currentLayout;
		...
		{
			StaticEventBus.dispatchEvent(new LoadLayoutEvent(ddl_layout.selectedItem as LayoutVO));
		}
		
		protected function ddl_style_changeHandler(event:IndexChangeEvent):void
		{
			StaticEventBus.dispatchEvent(new LoadStyleEvent(ddl_style.selectedItem as StyleVO));
		}
		
		protected function ddl_locale_changeHandler(event:IndexChangeEvent):void
		{
			StaticEventBus.dispatchEvent(new LoadLocaleEvent(ddl_locale.selectedItem as LocaleVO));
		}
