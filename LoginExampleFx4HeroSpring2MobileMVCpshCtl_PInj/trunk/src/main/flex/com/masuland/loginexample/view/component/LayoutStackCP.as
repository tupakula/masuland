	...
	public class LayoutStackCP extends SkinnableComponent
	{
		...
		protected function cp_resizeHandler(event:ResizeEvent):void
		{
			if (_currentLayout != null)
			{
				if (width < height)
				{
					StaticEventBus.dispatchEvent(new LoadLayoutEvent(_settings.layouts.getItemAt(0) as LayoutVO, false));
				}
				else
				{
					StaticEventBus.dispatchEvent(new LoadLayoutEvent(_settings.layouts.getItemAt(1) as LayoutVO, false));
				}
