package com.masuland.loginexample.view.component
{
	...
	public class SettingsBoxCP extends SkinnableComponent
	{
		...
		public function postConstruct():void
		{
			...
			ChangeWatcher.watch(appModel, 'currentStyle', currentStyle_changeHandler);
			ChangeWatcher.watch(appModel, 'currentLocale', currentLocale_changeHandler);
		}
...
//				ddl_layout.selectedItem = _currentLayout;
		...
		protected function currentStyle_changeHandler(event:Event):void
		{
			_currentStyle = appModel.currentStyle;
			
			_isCurrentStyleDirty = true;
			invalidateProperties();
		}
		
		protected function currentLocale_changeHandler(event:Event):void
		{
			_currentLocale = appModel.currentLocale;
		...
		{
			loadLayoutSignal.dispatch(ddl_layout.selectedItem as LayoutVO);
		}
		
		protected function ddl_style_changeHandler(event:IndexChangeEvent):void
		{
			loadStyleSignal.dispatch(ddl_style.selectedItem as StyleVO);
		}
		
		protected function ddl_locale_changeHandler(event:IndexChangeEvent):void
		{
			loadLocaleSignal.dispatch(ddl_locale.selectedItem as LocaleVO);
		}
